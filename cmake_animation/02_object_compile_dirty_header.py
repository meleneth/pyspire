...
sprites["secondary_src"].layers[Layer.DIRTY_INDICATOR.value].opacity = 0.0
sprites["main_src"].layers[Layer.DIRTY_INDICATOR.value].opacity = 0.0
...
make_program_sprite("some_program", "program.png",  "some_program.png",     Vec2(1550, 800))
...
# ---------------- Act Preamble: dirty header dirties sources ----------------
initial_bump = make_bump(sprites["secondary_header"], sprites["main_src"])
initial_bump.bus.on("bump_contact", lambda **kw: main_src_gets_dirty())
initial_bump.bus.on("bump_completed", lambda **kw: make_secondary_source_dirty())

def main_src_gets_dirty():
    fade = Fade(target=sprites['main_src'].layers[Layer.DIRTY_INDICATOR.value], duration_s=TIMING_FADE, start=0.0, end=1.0)
    animation.add_animation(fade)

def make_secondary_source_dirty():
    bump = make_bump(sprites["secondary_header"], sprites["secondary_src"])
    bump.bus.on("bump_contact", lambda **kw: secondary_src_gets_dirty())
    bump.bus.on("bump_completed", lambda **kw: start_the_build())

def secondary_src_gets_dirty():
    fade = Fade(target=sprites['secondary_src'].layers[Layer.DIRTY_INDICATOR.value], duration_s=TIMING_FADE, start=0.0, end=1.0)
    animation.add_animation(fade)
...
def on_secondary_object_bump_contact():
    fade = Fade(target=sprites['secondary_object'], duration_s=TIMING_FADE, start=0.0, end=1.0)
    animation.add_animation(fade)
...
def on_secondary_object_fade_in_completed():
    ...
    bump.bus.on("bump_contact", lambda **kw: on_main_object_bump_contact_linking())

# ---------------- Act 3: gpp -> main_object ----------------

def on_main_object_bump_contact_linking():
    bump = make_bump(sprites["main_object"], sprites["secondary_object"])
    bump.bus.on("bump_contact", lambda **kw: on_secondary_object_bump_contact_linking())

def on_secondary_object_bump_contact_linking():
    bump = make_bump(sprites["secondary_object"], sprites["some_program"])
    bump.bus.on("bump_contact", lambda **kw: fade_in_some_program())

def fade_in_some_program():
    fade = Fade(target=sprites['some_program'], duration_s=TIMING_FADE, start=0.0, end=1.0)
    animation.add_animation(fade)
